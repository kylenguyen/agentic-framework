...
|---|---|---|---|
| `install-host.sh` | host | the host login; `sudo` per command, only for phase 1 steps not yet done | phase 1: apt packages, zsh as login shell, linger, Tailscale auto-update, unattended-upgrades; then symlinked configs, secrets file skeleton, `xclip` shim, oh-my-zsh, mise toolchains, uv, the three harnesses |
...
installs tmux, mosh, gh, zsh, git, curl, file, jq, unattended-upgrades; sets zsh as the login shell; enables linger;
